	...
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
		...
		<script src="rico.js" type="text/javascript"></script>
         <script type="text/javascript">
             function ajax(num) {
                 var ajaxRequest = new AjaxRequest('PostBasecampTodoVP.do'); //Create AjaxRequest object
				 ajaxRequest.addFormElementsById(num + '_id');
				 ajaxRequest.addFormElementsById(num + '_ajax');
		 		 ajaxRequest.addFormElementsById(num + '_content');
		 		 ajaxRequest.addFormElementsById(num + '_ordernum');
		 		 ajaxRequest.addFormElementsById(num + '_basecampTodoList');
...
<span class="title">一覧</span>
                    <display:table name="${basecampTodos}" id="row" requestURI="AjaxDisplayBasecampTodos.do"
...
<nobr><input size="8" type="text" name="content" id="${row.id}_content" value="${row.content}" /></nobr>
</display:column>
...
<nobr><input size="8" type="text" name="ordernum" id="${row.id}_ordernum" value="${row.ordernum}" /></nobr>
 ...
 <display:column media="html" sortable="true" title="basecampTodoList">
                                <c:choose>
                                    ...
                                    <c:otherwise>
<select name="basecampTodoList" id="${row.id}_basecampTodoList"><option value=""></option>
                                            <c:forEach var="valueLabel" items="${BasecampTodoLists}">
                                                <c:choose>
                                                    <c:when test="${valueLabel.id eq row.basecampTodoList.id}">
