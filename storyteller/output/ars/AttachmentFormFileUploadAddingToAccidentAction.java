...
public class AttachmentFormFileUploadAddingToAccidentAction extends Action{
...
//			new HTTPGetRedirection(req, res, "##secoundmodelclass##Detail.do", attachmentform.getAccident().toString());
			req.setAttribute("id",attachmentform.getAccident());
			return mapping.findForward("error");
		}
		
               	Integer AccidentInt = attachmentform.getAccident();
		attachmentform.setAccident(null);

		Session session = new HibernateSession().currentSession(this
				.getServlet().getServletContext());
		Criteria criteria2 = session.createCriteria(Accident.class);
		criteria2.add(Restrictions.idEq(AccidentInt));
		Accident accident = (Accident) criteria2
				.uniqueResult();

                Attachment prevobj = accident.getAttachment();

                if(prevobj!=null){
                   prevobj.setAccident(null);
		...
		attachment.setAccident(accident);
		...
		new HTTPGetRedirection(req, res, "AccidentDetail.do", AccidentInt.toString());
		return null;
