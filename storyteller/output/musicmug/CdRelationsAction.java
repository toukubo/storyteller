...
public class CdRelationsAction extends Action{
	public ActionForward execute(
		...
		for (Iterator iter = criteria.list().iterator(); iter.hasNext();) {
			CdRelation cdRelation = (CdRelation) iter.next();
			vector.add(cdRelation);
		}
		CdRelation cdRelation = new CdRelationImpl();
		CdRelationForm cdRelationform = new CdRelationForm();
		criteria = session.createCriteria(CdRelation.class);
		...
		req.setAttribute("cdRelations",vector);
