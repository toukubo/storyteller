...
<html:checkbox name="form" property="original" /><br />
                                ...
                                partnerMedia<br />
                                <c:choose>
                                    <c:when test="${empty PartnerMedias}">
                                         <select name="partnerMedia" disabled="disabled"/>
                                    </c:when>
                                    <c:otherwise>
                                        <select name="partnerMedia" ><option value=""></option>
                                            <c:forEach var="valueLabel" items="${PartnerMedias}">
                                                <c:choose>
                                                    <c:when test="${valueLabel.id eq model.partnerMedia.id}">
                                                        <option value="${valueLabel.id}" selected="selected">${valueLabel.name}</option>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <option value="${valueLabel.id}">${valueLabel.name}</option>
