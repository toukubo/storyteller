...
public class UpdateArtistFromForSettingStatusPublishedAction extends Action{
	...
	public static void main(String[] args) {
		try {
				...
				UpdateArtistFromForSettingStatusPublishedAction action = new UpdateArtistFromForSettingStatusPublishedAction();
				Criteria criteria = session.createCriteria(Artist.class);
				criteria.add(Restrictions.idEq(new Integer(1)));
				Artist artist = (Artist) criteria.uniqueResult();
				action.exectute(artist, session);
				session.close();
