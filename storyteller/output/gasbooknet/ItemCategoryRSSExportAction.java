...
public class ItemCategoryRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = itemCategory.get##secoundclass##();
	    			criteria = session.createCriteria(ItemCategory.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("ItemCategorys");
	    			...
	    			newChannel.setDescription("ItemCategorys");
	    			...
	    			for (Iterator iter = criteria.list().iterator(); iter.hasNext();) {
	    				ItemCategory itemCategory = (ItemCategory)iter.next();
					ItemIF item = new Item();
					//item.setDate(new Date());

	    				item.setTitle(itemCategory.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(itemCategory.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("ItemCategoryRssExport.xml")),"utf-8");
