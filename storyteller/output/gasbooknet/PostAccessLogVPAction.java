package net.gasbooknet.web.app;

import net.gasbooknet.model.*;
import net.gasbooknet.beans.*;

import java.io.File;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import net.enclosing.util.StringFullfiller;
import net.enclosing.util.HTTPGetRedirection;
import net.enclosing.util.HibernateSession;

import org.apache.commons.beanutils.BeanUtils;
import org.apache.commons.lang.StringUtils;

import org.apache.struts.action.Action;
import org.apache.struts.action.ActionForm;
import org.apache.struts.action.ActionForward;
import org.apache.struts.action.ActionMapping;
import org.hibernate.Criteria;
import org.hibernate.Session;
import org.hibernate.Transaction;
import org.hibernate.criterion.Restrictions;


public class PostAccessLogVPAction extends Action{
	public ActionForward execute(
			ActionMapping mapping,
			ActionForm form,
			HttpServletRequest req,
			HttpServletResponse res) throws Exception{
		
		AccessLog accessLog = new AccessLogImpl();
		AccessLogForm accessLogform = (AccessLogForm) form;

                String error = "";


                		Integer publicUserInt = accessLogform.getPublicUser();
		accessLogform.setPublicUser(null);

		
		Session session = new HibernateSession().currentSession(this.getServlet().getServletContext());
		if(accessLogform.getId() == null || accessLogform.getId().intValue() == 0){
			accessLog.setId(null);
			accessLogform.setId(null);
		}else{
			accessLog.setId(accessLogform.getId());
			Criteria criteria = session.createCriteria(AccessLog.class);
			criteria.add(Restrictions.idEq(Integer.valueOf(req.getParameter("id"))));
			accessLog = (AccessLog) criteria.uniqueResult();

		}
                		accessLog.setDate(accessLogform.getDate());
		accessLog.setUri(accessLogform.getUri());
		accessLog.setModel(accessLogform.getModel());
		accessLog.setPagetitle(accessLogform.getPagetitle());


		
		StringFullfiller.fullfil(accessLog);
                		Criteria criteria2 = session.createCriteria(PublicUser.class);
		criteria2.add(Restrictions.idEq(publicUserInt));
		PublicUser publicuser = (PublicUser) criteria2.uniqueResult();
		accessLog.setPublicUser(publicuser);


		Transaction transaction = session.beginTransaction();
		session.saveOrUpdate(accessLog);
		transaction.commit();
		session.flush();
                if(req.getParameter("from")!=null && req.getParameter("from").equals("detail")){
		   new HTTPGetRedirection(req, res, "PostAccessLogDetail.do", accessLog.getId().toString());
                }
		if (StringUtils.isNotBlank(req.getParameter("ajax"))) {
			req.setAttribute("message","success");
			return mapping.findForward("success");
		}

		
		new HTTPGetRedirection(req, res, "AccessLogs.do", accessLog.getId().toString());
		return null;

		
	}
	
	
}