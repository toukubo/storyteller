...
public class UpdateWorkForFillingColorsAction extends Action{
	...
	public static void main(String[] args) {
		try {
				...
				UpdateWorkForFillingColorsAction action = new UpdateWorkForFillingColorsAction();
				Criteria criteria = session.createCriteria(Work.class);
				criteria.add(Restrictions.idEq(new Integer(1)));
				Work work = (Work) criteria.uniqueResult();
				action.execute(work, session);
				session.close();
