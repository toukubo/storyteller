...
public class AccessLogRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = accessLog.get##secoundclass##();
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("AccessLogs");
	    			...
	    			newChannel.setDescription("AccessLogs");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(accessLog.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(accessLog.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("AccessLogRssExport.xml")),"utf-8");
