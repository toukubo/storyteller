...
public class XlsExportOfEventAction{
	public XlsExportOfEventAction(Vector vector,OutputStream outputStream,Session session){
        ...
        try{
    		...
    		for (Iterator iter = vector.iterator(); iter.hasNext();) {
    			...
    			sheet.addCell(new Label(x++, y, event.get().getTypename()));
    			sheet.addCell(new Label(x++, y, event.get().getModelcode()));
    			sheet.addCell(new Label(x++, y, event.get().getShimuke()));
    			sheet.addCell(new Label(x++, y, event.get().getName()));
    			sheet.addCell(new Number(x++, y, event.get()));
    			sheet.addCell(new Number(x++, y, event.get()));
