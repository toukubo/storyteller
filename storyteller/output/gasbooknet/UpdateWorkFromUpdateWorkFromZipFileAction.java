...
public class UpdateWorkFromUpdateWorkFromZipFileAction extends Action{
	...
	public static void main(String[] args) {
		try {
				...
				UpdateWorkFromUpdateWorkFromZipFileAction action = new UpdateWorkFromUpdateWorkFromZipFileAction();
				Criteria criteria = session.createCriteria(Work.class);
				criteria.add(Restrictions.idEq(new Integer(1)));
				Work work = (Work) criteria.uniqueResult();
				action.exectute(work, session);
				session.close();
