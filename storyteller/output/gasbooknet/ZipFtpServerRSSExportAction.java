...
public class ZipFtpServerRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = zipFtpServer.get##secoundclass##();
	    			criteria = session.createCriteria(ZipFtpServer.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("ZipFtpServers");
	    			...
	    			newChannel.setDescription("ZipFtpServers");
	    			...
	    			for (Iterator iter = criteria.list().iterator(); iter.hasNext();) {
	    				ZipFtpServer zipFtpServer = (ZipFtpServer)iter.next();
					ItemIF item = new Item();
					//item.setDate(new Date());

	    				item.setTitle(zipFtpServer.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(zipFtpServer.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("ZipFtpServerRssExport.xml")),"utf-8");
