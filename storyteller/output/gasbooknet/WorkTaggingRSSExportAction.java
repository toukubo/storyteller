...
public class WorkTaggingRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = workTagging.get##secoundclass##();
	    			criteria = session.createCriteria(WorkTagging.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("WorkTaggings");
	    			...
	    			newChannel.setDescription("WorkTaggings");
	    			...
	    			for (Iterator iter = criteria.list().iterator(); iter.hasNext();) {
	    				WorkTagging workTagging = (WorkTagging)iter.next();
					ItemIF item = new Item();
					//item.setDate(new Date());

	    				item.setTitle(workTagging.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(workTagging.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("WorkTaggingRssExport.xml")),"utf-8");
