...
public class SetSelectionOfNewsTaggingNewsAction extends Action{
	public ActionForward execute(
		...
		for (int I = 0; I < req.getParameterValues("id").length; i++) {
			...
			NewsTagging newsTagging = (NewsTagging)criteria2.uniqueResult();
			newsTagging.setnews(true);
			session.save(newsTagging);
		}
		transaction.commit();
		session.flush();
