...
public class XlsShowArtistAction extends Action{
	public ActionForward execute(
		...
		Map map = new HashMap();
		map.put("artist.name",artist.getName());
		map.put("artist.attr1",artist.getAttr1());
		
		
		File file = new File(this.getServlet().getServletContext().getRealPath("XlsShowArtist.xls"));
		res.setHeader("Pragma", ""); 
		res.setHeader("Cache-Control", ""); 
		res.setHeader("Content-Disposition","attachment; filename=XlsShowArtist.xls");
