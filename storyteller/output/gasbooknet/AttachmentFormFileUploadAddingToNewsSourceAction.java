...
public class AttachmentFormFileUploadAddingToNewsSourceAction extends Action{
...
//			new HTTPGetRedirection(req, res, "##secoundmodelclass##Detail.do", attachmentform.getNewsSource().toString());
			req.setAttribute("id",attachmentform.getNewsSource());
			return mapping.findForward("error");
		}
		
		Integer NewsSourceInt = Integer.valueOf(req.getParameter("newsSource"));
		
//		attachment.setNumber(Integer.parseInt(req.getParameter("setNumber")));
		
		Session session = new HibernateSession().currentSession(this
				.getServlet().getServletContext());
		Criteria criteria2 = session.createCriteria(NewsSource.class);
		criteria2.add(Restrictions.idEq(NewsSourceInt));
		NewsSource newsSource = (NewsSource) criteria2
				.uniqueResult();
//		attachment.setNewsSource(newsSource);
		...
		new HTTPGetRedirection(req, res, "PostNewsSourceDetail.do", NewsSourceInt.toString());
		return null;
