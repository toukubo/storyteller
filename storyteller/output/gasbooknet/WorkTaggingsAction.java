...
public class WorkTaggingsAction extends Action{
	public ActionForward execute(
		...
		for (Iterator iter = criteria.list().iterator(); iter.hasNext();) {
			...
			vector.add(workTagging);
		}
		WorkTagging workTagging = new WorkTaggingImpl();
		Criteria criteria2 = session.createCriteria(WorkTagging.class);

		if (req.getParameter("id") != null
				&& !req.getParameter("id").equals("")) {
			criteria2.add(Restrictions.idEq(Integer.valueOf(req
					.getParameter("id"))));
			workTagging = (WorkTagging) criteria2.uniqueResult();
		...
		req.setAttribute("workTaggings",vector);
