...
public class ArtistTaggingRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = artistTagging.get##secoundclass##();
	    			criteria = session.createCriteria(ArtistTagging.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("ArtistTaggings");
	    			...
	    			newChannel.setDescription("ArtistTaggings");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(artistTagging.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(artistTagging.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("ArtistTaggingRssExport.xml")),"utf-8");
