...
public class SetSelectionOfArtistTaggingArtistAction extends Action{
	public ActionForward execute(
		...
		for (int I = 0; I < req.getParameterValues("id").length; i++) {
			...
			ArtistTagging artistTagging = (ArtistTagging)criteria2.uniqueResult();
			artistTagging.setartist(true);
			session.save(artistTagging);
		}
		transaction.commit();
		session.flush();
