...
public class NewsTaggingRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = newsTagging.get##secoundclass##();
	    			criteria = session.createCriteria(NewsTagging.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("NewsTaggings");
	    			...
	    			newChannel.setDescription("NewsTaggings");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(newsTagging.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(newsTagging.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("NewsTaggingRssExport.xml")),"utf-8");
