...
public class NewsTypeRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = newsType.get##secoundclass##();
	    			criteria = session.createCriteria(NewsType.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("NewsTypes");
	    			...
	    			newChannel.setDescription("NewsTypes");
	    			...
	    			for (Iterator iter = criteria.list().iterator(); iter.hasNext();) {
	    				NewsType newsType = (NewsType)iter.next();
					ItemIF item = new Item();
					//item.setDate(new Date());

	    				item.setTitle(newsType.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(newsType.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("NewsTypeRssExport.xml")),"utf-8");
