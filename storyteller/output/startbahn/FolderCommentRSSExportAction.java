...
public class FolderCommentRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = folderComment.get##secoundclass##();
	    			criteria = session.createCriteria(FolderComment.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("FolderComments");
	    			...
	    			newChannel.setDescription("FolderComments");
	    			...
	    			for (Iterator iter = criteria.list().iterator(); iter.hasNext();) {
	    				FolderComment folderComment = (FolderComment)iter.next();
					ItemIF item = new Item();
					//item.setDate(new Date());

	    				item.setTitle(folderComment.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(folderComment.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("FolderCommentRssExport.xml")),"utf-8");
