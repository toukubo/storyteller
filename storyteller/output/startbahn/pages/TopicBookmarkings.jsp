...
<table align="right">
	...
	<div>	
		<td>
			<form action="GrepTopicBookmarking.do">
...
<div>
<form action="TopicBookmarkings.do">
			...
			<c:choose>
				...
				<c:forEach var="valueLabel" items="${PublicUsers}">
					<option value="${valueLabel.id}">${valueLabel.name}</option>
		                     ...
		                     &nbsp;
			<c:choose>
				<c:when test="${empty ForumTopics}">
					<select name="forumTopic" disabled="disabled"/>
				</c:when>
				<c:otherwise>
				<select name="forumTopic" ><option value="">All</option>
				<c:forEach var="valueLabel" items="${ForumTopics}">
					<option value="${valueLabel.id}">${valueLabel.name}</option>
				</c:forEach>
				</select>
				</c:otherwise>
...
<input type="submit" value="Go" onclick="document.publicusers1.action='PublicUsers.do'"> 
                    ...
                    <display:table name="${topicBookmarkings}" id="row" requestURI="TopicBookmarkings.do"
                            requestURIcontext="false"
                            export="false" pagesize="30" sort="list">
                        <display:column media="html" sortable="false">
                        <a href="ShowTopicBookmarking.do?id=${row.id}">Show</a> | 
                        <a href="PostTopicBookmarkingDetail.do?id=${row.id}"><img src="icons/table_edit.png"></a>
                        <a href="DeleteTopicBookmarking.do?id=${row.id}">削除</a>
                        </display:column>
