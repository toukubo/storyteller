...
public class BettingRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = betting.get##secoundclass##();
	    			criteria = session.createCriteria(Betting.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("Bettings");
	    			...
	    			newChannel.setDescription("Bettings");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(betting.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(betting.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("BettingRssExport.xml")),"utf-8");
