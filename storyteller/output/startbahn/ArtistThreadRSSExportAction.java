...
public class ArtistThreadRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = artistThread.get##secoundclass##();
	    			criteria = session.createCriteria(ArtistThread.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("ArtistThreads");
	    			...
	    			newChannel.setDescription("ArtistThreads");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(artistThread.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(artistThread.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("ArtistThreadRssExport.xml")),"utf-8");
