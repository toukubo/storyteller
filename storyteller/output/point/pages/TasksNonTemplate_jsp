...
<input type="submit" value="Go" onclick="document.publicusers1.action='PublicUsers.do'"> 
                    ...
                    <display:table name="${tasks}" id="row" requestURI="Tasks.do"
                        ...
                        <a href="ShowTask.do?id=${row.id}">Show</a> | 
                        <a href="PostTaskDetail.do?id=${row.id}"><img src="images/icons/table_edit.png"></a>
                        <a onClick="javascript:if(window.confirm('削除してもよろしいですか？')){location.href='DeleteTask.do?id=${row.id}';}">
...
<bean:write name="row" property="date" format="yyyy/MM/dd" /> </nobr>
 ...
 <display:column media="html" sortable="true" title="template">
<nobr>
${row.template}
</nobr>
</display:column>
 <display:column media="html" sortable="true" title="duration">
<nobr>
${row.duration}
</nobr>
</display:column>
