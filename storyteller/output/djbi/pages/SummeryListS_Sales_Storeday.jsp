...
$('table.displayArea').toggle();
	...
	<tiles:put name="content" direct="true">
		...
		<div>
		<form action="S_Sales_Storedays.do"　method="post">
...
-->					
			...
			<tbody>
				<c:set var="object" value="${totals_Sales_Storeday}"/>
				<jsp:useBean id="object" class="net.##nameofj2eestory##.model.S_Sales_StoredayImpl" scope="page"/>
					...
					<td width="50px">
						<c:choose>
							<c:when test="${param.unit eq '1000'}">
								${totals_Sales_Storeday.ordertotal/param.unit}		
							</c:when>
							<c:otherwise>
								${totals_Sales_Storeday.ordertotal}						
							</c:otherwise>
						</c:choose>
					</td>
					<td width="50px">
						${totals_Sales_Storeday.ordernum}								
					</td>
					<td width="50px">
						<c:choose>
							<c:when test="${param.unit eq '1000'}">
								${totals_Sales_Storeday.deliverytotal/param.unit}		
							</c:when>
							<c:otherwise>
								${totals_Sales_Storeday.deliverytotal}
							</c:otherwise>
						</c:choose>
					</td>
					<td width="50px">
						${totals_Sales_Storeday.deliverynum}																
					</td>
					<td width="50px">
						${totals_Sales_Storeday.ordernum - totals_Sales_Storeday.deliverynum}
					</td>
					<td width="50px">
						<c:choose>
							<c:when test="${param.unit eq '1000'}">
								${(totals_Sales_Storeday.ordertotal - totals_Sales_Storeday.deliverytotal)/param.unit}		
							</c:when>
							<c:otherwise>
								${totals_Sales_Storeday.ordertotal - totals_Sales_Storeday.deliverytotal}
							</c:otherwise>
						</c:choose>
					</td>
					<td width="50px">
			...
			##summeryattributes##
				...
				<logic:iterate id="row" name="s_Sales_Storedays">
					<c:set var="obj" value="${row}"/>
					<jsp:useBean id="obj" class="net.nameofj2eestory.web.app.S_Sales_StoredayBean" scope="page"/>
						<c:choose>
						...
						<td width="50px">
							<c:choose>
								<c:when test="${param.unit eq '1000'}">
									${row.s_Sales_Storeday.ordertotal/param.unit}		
								</c:when>
								<c:otherwise>
									${row.s_Sales_Storeday.ordertotal}						
								</c:otherwise>
							</c:choose>
						</td>
						<td width="50px">
							${row.s_Sales_Storeday.ordernum}								
						</td>
						<td width="50px">
							<c:choose>
								<c:when test="${param.unit eq '1000'}">
									${row.s_Sales_Storeday.deliverytotal/param.unit}		
								</c:when>
								<c:otherwise>
									${row.s_Sales_Storeday.deliverytotal}
								</c:otherwise>
							</c:choose>
						</td>
						<td width="50px">
							${row.s_Sales_Storeday.deliverynum}																
						</td>
						<td width="50px">
							${row.s_Sales_Storeday.ordernum - row.s_Sales_Storeday.deliverynum}
						</td>
						<td width="50px">
							<c:choose>
								<c:when test="${param.unit eq '1000'}">
									${(row.s_Sales_Storeday.ordertotal - row.s_Sales_Storeday.deliverytotal)/param.unit}		
								</c:when>
								<c:otherwise>
									${row.s_Sales_Storeday.ordertotal - row.s_Sales_Storeday.deliverytotal}
								</c:otherwise>
							</c:choose>
						</td>
						<td width="50px">
							<%= Calculate.rate(obj.getSupplierDeliverys_Sales_Storeday(),"total") %>
						</td>
						<td width="50px" style="padding-right:15px;">
							<%= Calculate.rate(obj.getSupplierDeliverys_Sales_Storeday(),"num") %>
						</td>
					</tr>
				</logic:iterate>		
		</tbody>
