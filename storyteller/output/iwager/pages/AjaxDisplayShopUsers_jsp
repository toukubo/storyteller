	...
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
		...
		<script src="rico.js" type="text/javascript"></script>
         <script type="text/javascript">
             function ajax(num) {
                 var ajaxRequest = new AjaxRequest('PostShopUserVP.do'); //Create AjaxRequest object
				 ...
				 ajaxRequest.addFormElementsById(num + '_ajax');
		 		 ...
		 		 ajaxRequest.addFormElementsById(num + '_removed');
		 		 ajaxRequest.addFormElementsById(num + '_shop');
		 		 ajaxRequest.addFormElementsById(num + '_name');
		 		 ajaxRequest.addFormElementsById(num + '_flag');
		 		 ajaxRequest.addFormElementsById(num + '_date');
...
<div id="message"></div>
                    <display:table name="${shopUsers}" id="row" requestURI="AjaxDisplayShopUsers.do"
                        ...
                        <a href="DeleteShopUser.do?id=${row.id}">削除</a>
                        </display:column>
...
<nobr><input size="8" type="text" name="starthour" id="${row.id}_starthour" value="${row.starthour}" /></nobr>
 ...
 <display:column media="html" sortable="true" title="removed">
<nobr><input size="8" type="text" name="removed" id="${row.id}_removed" value="${row.removed}" /></nobr>
</display:column>
 <display:column media="html" sortable="true" title="shop">
                                <c:choose>
                                    ...
                                    <c:otherwise>
<select name="shop" id="${row.id}_shop"><option value=""></option>
                                            <c:forEach var="valueLabel" items="${Shops}">
                                                <c:choose>
                                                    <c:when test="${valueLabel.id eq row.shop.id}">
...
<nobr><input size="8" type="text" name="flag" id="${row.id}_flag" value="${row.flag}" /></nobr>
 ...
 <display:column media="html" sortable="true" title="date"><nobr><input size="8" type="text" name="dateAsString" id="${row.id}_date" value="<st:date date="${row.basedeadline}"></st:date>" /></nobr>
</display:column>
...
<span id="subform">
		...
		<form method="POST" action="PostShopUserVP.do">
