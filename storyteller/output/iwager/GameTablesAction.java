...
public class GameTablesAction extends Action{
	public ActionForward execute(
		...
		for (Iterator iter = criteria.list().iterator(); iter.hasNext();) {
			GameTable gameTable = (GameTable) iter.next();
			vector.add(gameTable);
		}
		GameTable gameTable = new GameTableImpl();
		GameTableForm gameTableform = new GameTableForm();
		criteria = session.createCriteria(GameTable.class);
		...
		req.setAttribute("gameTables",vector);
...
Criteria criteriaPit= session.createCriteria(Pit.class);
			req.setAttribute("Pits", criteriaPit.list());
