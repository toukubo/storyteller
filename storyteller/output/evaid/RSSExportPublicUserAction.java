...
public class RSSExportPublicUserAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = publicUser.get##secoundclass##();
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("PublicUsers");
	    			...
	    			newChannel.setDescription("PublicUsers");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(publicUser.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(publicUser.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("PublicUserRssExport.xml")),"utf-8");
