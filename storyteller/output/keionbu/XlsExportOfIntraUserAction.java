	...
	public XlsExportOfIntraUserAction(Vector vector,OutputStream outputStream,Session session){
        ...
        try{
    		...
    		for (Iterator iter = vector.iterator(); iter.hasNext();) {
    			...
    			sheet.addCell(new Label(x++, y, intraUser.get().getTypename()));
    			sheet.addCell(new Label(x++, y, intraUser.get().getModelcode()));
    			sheet.addCell(new Label(x++, y, intraUser.get().getShimuke()));
    			sheet.addCell(new Label(x++, y, intraUser.get().getName()));
    			sheet.addCell(new Number(x++, y, intraUser.get()));
    			sheet.addCell(new Number(x++, y, intraUser.get()));
