...
public class SetSelectionOfInstrumentIntraUsersAction extends Action{
	public ActionForward execute(
		...
		for (int I = 0; I < req.getParameterValues("id").length; i++) {
			Criteria criteria2 = session.createCriteria(Instrument.class);
			criteria2.add(Restrictions.idEq(Integer.valueOf(req.getParameterValues("id")[i])));
			Instrument instrument = (Instrument)criteria2.uniqueResult();
			instrument.setintraUsers(true);
