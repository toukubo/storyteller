...
public class SetSelectionOfPaymentWithholdingtaxAction extends Action{
	public ActionForward execute(
		...
		for (int I = 0; I < req.getParameterValues("id").length; i++) {
			Criteria criteria2 = session.createCriteria(Payment.class);
			criteria2.add(Restrictions.idEq(Integer.valueOf(req.getParameterValues("id")[i])));
			Payment payment = (Payment)criteria2.uniqueResult();
			payment.setwithholdingtax(true);
