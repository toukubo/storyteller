...
public class PaymentRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = payment.get##secoundclass##();
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("Payments");
	    			...
	    			newChannel.setDescription("Payments");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(payment.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(payment.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("PaymentRssExport.xml")),"utf-8");
