...
public class ContractRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = contract.get##secoundclass##();
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("Contracts");
	    			...
	    			newChannel.setDescription("Contracts");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(contract.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(contract.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("ContractRssExport.xml")),"utf-8");
