	...
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
		...
		<script src="rico.js" type="text/javascript"></script>
         <script type="text/javascript">
             function ajax(num) {
                 var ajaxRequest = new AjaxRequest('PostBasecampAccountVP.do'); //Create AjaxRequest object
				 ajaxRequest.addFormElementsById(num + '_id');
				 ajaxRequest.addFormElementsById(num + '_ajax');
		 		 ajaxRequest.addFormElementsById(num + '_basecampProjects');
		 		 ajaxRequest.addFormElementsById(num + '_apid');
...
<span class="title">一覧</span>
                    <display:table name="${basecampAccounts}" id="row" requestURI="AjaxDisplayBasecampAccounts.do"
...
<nobr><input size="8" type="text" name="apid" id="${row.id}_apid" value="${row.apid}" /></nobr>
</display:column>
...
apid<br />
<html:text name="form" property="apid" /><br />
