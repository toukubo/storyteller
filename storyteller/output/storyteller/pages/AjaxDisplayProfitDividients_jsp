	...
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
		...
		<script src="rico.js" type="text/javascript"></script>
         <script type="text/javascript">
             function ajax(num) {
                 var ajaxRequest = new AjaxRequest('PostProfitDividientVP.do'); //Create AjaxRequest object
...
<span class="title">利潤分配一覧</span>
                    <display:table name="${profitDividients}" id="row" requestURI="ProfitDividients.do"
                        ...
                        <a href="DeleteProfitDividient.do?id=${row.id}">削除</a>
                        </display:column>
...
<nobr><input size="8" onblur="ajax(${row.id});" type="text" name="payed" id="${row.id}_payed" value="${row.payed}" /></nobr>
</display:column>
...
<span id="subform">
		...
		<form method="POST" action="PostProfitDividientVP.do">
...
<html:checkbox name="form" property="payed" /><br />
          ...
          <html:submit value="save" />
                        ...
                        <a href="ProfitDividients.do">新規利潤分配作成</a>
</div>
