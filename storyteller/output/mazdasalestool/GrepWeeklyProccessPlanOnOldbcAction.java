...
public class GrepWeeklyProccessPlanOnOldbcAction extends Action{
	public ActionForward execute(
		...
		Criteria criteria = session.createCriteria(WeeklyProccessPlan.class);
	    	if(req.getParameter("q") !=null && !req.getParameter("q").equals("")){
	    	   criteria.add(Restrictions.like("oldbc","%" + new String(req.getParameter("q").getBytes("8859_1"), "UTF-8")  + "%"));
	  	}
		session.flush();
		req.setAttribute("intrausers", criteria.list());
		req.setAttribute("from","GrepWeeklyProccessPlanOnOldbc");
