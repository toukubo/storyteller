...
public class GrepResultAndProductivityOnActivityslcAction extends Action{
	public ActionForward execute(
		...
		Criteria criteria = session.createCriteria(ResultAndProductivity.class);
	    	if(req.getParameter("q") !=null && !req.getParameter("q").equals("")){
	    	   criteria.add(Restrictions.like("activityslc","%" + new String(req.getParameter("q").getBytes("8859_1"), "UTF-8")  + "%"));
	  	}
		session.flush();
		req.setAttribute("intrausers", criteria.list());
		req.setAttribute("from","GrepResultAndProductivityOnActivityslc");
