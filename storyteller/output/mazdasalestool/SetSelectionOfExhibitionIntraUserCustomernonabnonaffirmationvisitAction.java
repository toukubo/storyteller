...
public class SetSelectionOfExhibitionIntraUserCustomernonabnonaffirmationvisitAction extends Action{
	public ActionForward execute(
		...
		for (int I = 0; I < req.getParameterValues("id").length; i++) {
			...
			exhibitionIntraUser.setcustomernonabnonaffirmationvisit(true);
			session.save(exhibitionIntraUser);
		}
		transaction.commit();
		session.flush();
