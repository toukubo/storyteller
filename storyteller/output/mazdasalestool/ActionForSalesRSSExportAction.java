...
public class ActionForSalesRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = actionForSales.get##secoundclass##();
	    			criteria = session.createCriteria(ActionForSales.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("ActionForSaless");
	    			...
	    			newChannel.setDescription("ActionForSaless");
	    			...
	    			for (Iterator iter = criteria.list().iterator(); iter.hasNext();) {
	    				ActionForSales actionForSales = (ActionForSales)iter.next();
					ItemIF item = new Item();
					//item.setDate(new Date());

	    				item.setTitle(actionForSales.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(actionForSales.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("ActionForSalesRssExport.xml")),"utf-8");
