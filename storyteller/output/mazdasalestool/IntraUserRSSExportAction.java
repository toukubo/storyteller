...
public class IntraUserRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = intraUser.get##secoundclass##();
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("IntraUsers");
	    			...
	    			newChannel.setDescription("IntraUsers");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(intraUser.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(intraUser.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("IntraUserRssExport.xml")),"utf-8");
