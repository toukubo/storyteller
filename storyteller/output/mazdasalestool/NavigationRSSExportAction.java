...
public class NavigationRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = navigation.get##secoundclass##();
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("Navigations");
	    			...
	    			newChannel.setDescription("Navigations");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(navigation.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(navigation.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("NavigationRssExport.xml")),"utf-8");
