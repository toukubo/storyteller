...
public class PrefectureRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = prefecture.get##secoundclass##();
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("Prefectures");
	    			...
	    			newChannel.setDescription("Prefectures");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(prefecture.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(prefecture.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("PrefectureRssExport.xml")),"utf-8");
