...
if(StringUtils.isBlank(userform.getPassword())){
   error += "パスワードは空にはできません。<br />"; 
        ...
        if(!productform.getTypename().matches("[a-zA-Z0-9&/-]*")){
      	   error += "パスワードは半角英数字及び記号(/&-)のみです。<br />";
        }

if(StringUtils.isBlank(userform.getMail())){
...
if(StringUtils.isBlank(userform.getLoginid())){
   error += "ログインIDは空にはできません。<br />"; 
}

        if(!productform.getTypename().matches("[a-zA-Z0-9&/-]*")){
      	   error += "ログインIDは半角英数字及び記号(/&-)のみです。<br />";
        }
