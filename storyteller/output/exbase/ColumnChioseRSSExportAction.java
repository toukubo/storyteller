...
public class ColumnChioseRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = columnChiose.get##secoundclass##();
	    			criteria = session.createCriteria(ColumnChiose.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("ColumnChioses");
	    			...
	    			newChannel.setDescription("ColumnChioses");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(columnChiose.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(columnChiose.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("ColumnChioseRssExport.xml")),"utf-8");
