...
public class OperationRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = operation.get##secoundclass##();
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("Operations");
	    			...
	    			newChannel.setDescription("Operations");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(operation.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(operation.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("OperationRssExport.xml")),"utf-8");
