...
public class TransitionRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = transition.get##secoundclass##();
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("Transitions");
	    			...
	    			newChannel.setDescription("Transitions");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(transition.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(transition.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("TransitionRssExport.xml")),"utf-8");
