...
public class SetSelectionOfTransitionOperationAction extends Action{
	public ActionForward execute(
		...
		for (int I = 0; I < req.getParameterValues("id").length; i++) {
			Criteria criteria2 = session.createCriteria(Transition.class);
			criteria2.add(Restrictions.idEq(Integer.valueOf(req.getParameterValues("id")[i])));
			Transition transition = (Transition)criteria2.uniqueResult();
			transition.setoperation(true);
