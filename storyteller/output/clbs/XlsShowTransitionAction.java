...
public class XlsShowTransitionAction extends Action{
	public ActionForward execute(
		...
		Map map = new HashMap();
		map.put("transition.name",transition.getName());
		map.put("transition.attr1",transition.getAttr1());
		
		
		File file = new File(this.getServlet().getServletContext().getRealPath("XlsShowTransition.xls"));
		res.setHeader("Pragma", ""); 
		res.setHeader("Cache-Control", ""); 
		res.setHeader("Content-Disposition","attachment; filename=XlsShowTransition.xls");
