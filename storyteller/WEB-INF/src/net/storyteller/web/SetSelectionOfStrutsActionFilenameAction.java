...
public class SetSelectionOfStrutsActionIdAction extends Action{
	public ActionForward execute(
		...
		for (int I = 0; I < req.getParameterValues("id").length; i++) {
			...
			StrutsAction strutsaction = (StrutsAction)criteria2.uniqueResult();
			strutsaction.setid(true);
			session.save(strutsaction);
		}
		transaction.commit();
		session.flush();
