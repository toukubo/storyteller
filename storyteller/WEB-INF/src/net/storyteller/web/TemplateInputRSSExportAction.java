...
public class TemplateInputRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = templateinput.get##secoundclass##();
	    			criteria = session.createCriteria(TemplateInput.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("TemplateInputs");
	    			...
	    			newChannel.setDescription("TemplateInputs");
	    			...
	    			for (Iterator iter = criteria.list().iterator(); iter.hasNext();) {
	    				TemplateInput templateinput = (TemplateInput)iter.next();
					ItemIF item = new Item();
					//item.setDate(new Date());

	    				item.setTitle(templateinput.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(templateinput.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("TemplateInputRssExport.xml")),"utf-8");
