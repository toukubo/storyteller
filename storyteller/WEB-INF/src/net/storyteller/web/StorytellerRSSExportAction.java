...
public class StorytellerRSSExportAction extends Action{
	...
	public ActionForward execute(
 		...
 		Session session = new HibernateSession().currentSession(this
	    		...
	    		Criteria criteria = session.createCriteria(Storyteller.class);
	    		criteria.add(Restrictions.idEq(req.getAttribute("id")));
//	    		##secoundclass## ##secoundobj## = storyteller.get##secoundclass##();
	    			criteria = session.createCriteria(Storyteller.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("Storytellers");
	    			...
	    			newChannel.setDescription("Storytellers");
	    			...
	    			for (Iterator iter = criteria.list().iterator(); iter.hasNext();) {
	    				Storyteller storyteller = (Storyteller)iter.next();
					ItemIF item = new Item();
					//item.setDate(new Date());

	    				item.setTitle(storyteller.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(storyteller.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("StorytellerRssExport.xml")),"utf-8");
