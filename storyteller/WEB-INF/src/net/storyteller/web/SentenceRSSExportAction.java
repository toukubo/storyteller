...
public class SentenceRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = sentence.get##secoundclass##();
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("Sentences");
	    			...
	    			newChannel.setDescription("Sentences");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(sentence.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(sentence.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("SentenceRssExport.xml")),"utf-8");
