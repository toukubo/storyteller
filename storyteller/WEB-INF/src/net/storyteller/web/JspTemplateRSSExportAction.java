...
public class JspTemplateRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = jsptemplate.get##secoundclass##();
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("JspTemplates");
	    			...
	    			newChannel.setDescription("JspTemplates");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(jsptemplate.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(jsptemplate.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("JspTemplateRssExport.xml")),"utf-8");
