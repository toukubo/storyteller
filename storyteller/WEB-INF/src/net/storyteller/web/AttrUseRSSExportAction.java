...
public class AttrUseRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = attruse.get##secoundclass##();
	    			criteria = session.createCriteria(AttrUse.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("AttrUses");
	    			...
	    			newChannel.setDescription("AttrUses");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(attruse.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(attruse.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("AttrUseRssExport.xml")),"utf-8");
