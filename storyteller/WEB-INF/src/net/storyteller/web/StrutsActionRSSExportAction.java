...
public class StrutsActionRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = strutsaction.get##secoundclass##();
	    			criteria = session.createCriteria(StrutsAction.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("StrutsActions");
	    			...
	    			newChannel.setDescription("StrutsActions");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(strutsaction.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(strutsaction.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("StrutsActionRssExport.xml")),"utf-8");
