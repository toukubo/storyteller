...
public class NounClauseRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = nounclause.get##secoundclass##();
	    			criteria = session.createCriteria(NounClause.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("NounClauses");
	    			...
	    			newChannel.setDescription("NounClauses");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(nounclause.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(nounclause.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("NounClauseRssExport.xml")),"utf-8");
