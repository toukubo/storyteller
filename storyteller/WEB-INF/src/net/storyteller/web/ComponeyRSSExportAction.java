...
public class ComponeyRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = componey.get##secoundclass##();
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("Componeys");
	    			...
	    			newChannel.setDescription("Componeys");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(componey.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(componey.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("ComponeyRssExport.xml")),"utf-8");
