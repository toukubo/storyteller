...
public class NounUseRSSExportAction extends Action{
	...
	public ActionForward execute(
 		...
 		Session session = new HibernateSession().currentSession(this
	    		...
	    		Criteria criteria = session.createCriteria(NounUse.class);
	    		criteria.add(Restrictions.idEq(req.getAttribute("id")));
//	    		##secoundclass## ##secoundobj## = nounuse.get##secoundclass##();
	    			criteria = session.createCriteria(NounUse.class);
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("NounUses");
	    			...
	    			newChannel.setDescription("NounUses");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(nounuse.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(nounuse.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("NounUseRssExport.xml")),"utf-8");
