...
public class StaticDataRSSExportAction extends Action{
...
//	    		##secoundclass## ##secoundobj## = staticdata.get##secoundclass##();
...
//	    			criteria.add(Restrictions.eq("##secoundobj##", ##secoundobj##));
	    			...
	    			ChannelIF newChannel = builder.createChannel("StaticDatas");
	    			...
	    			newChannel.setDescription("StaticDatas");
					...
					//item.setDate(new Date());
	    				...
	    				item.setTitle(staticdata.getName());
	    				item.setLink(new URL("http://"+req.getServerName()+req.getRequestURI());
	    				item.setDescription(staticdata.getName());
	    			...
	    			PrintWriter printWriter = new PrintWriter(new File(this.getServlet().getServletContext().getRealPath("StaticDataRssExport.xml")),"utf-8");
