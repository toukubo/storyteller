	...
	public XlsExportOfJ2eeStoryAction(Vector vector,OutputStream outputStream,Session session){
        ...
        try{
    		...
    		for (Iterator iter = vector.iterator(); iter.hasNext();) {
    			...
    			sheet.addCell(new Label(x++, y, j2eestory.get().getTypename()));
    			sheet.addCell(new Label(x++, y, j2eestory.get().getModelcode()));
    			sheet.addCell(new Label(x++, y, j2eestory.get().getShimuke()));
    			sheet.addCell(new Label(x++, y, j2eestory.get().getName()));
    			sheet.addCell(new Number(x++, y, j2eestory.get()));
    			sheet.addCell(new Number(x++, y, j2eestory.get()));
